from conftest import open_advanced_mode_and_reload


def test_record_edit_page_loads(logged_in_page, base_url, created_record, created_account):  # created_record: fixture side-effect only — ensures a record exists in DB
...
def test_record_edit_success(logged_in_page, base_url, created_record, created_account):  # created_record: fixture side-effect only — ensures a record exists in DB
    logged_in_page.goto(f"{base_url}/record/alter/edit")
    open_advanced_mode_and_reload(logged_in_page, created_account, checked="0")
    logged_in_page.locator("a[href*='/record/edit/']").first.click()  # [href*=] = CSS "contains": matches /record/edit/1, /record/edit/42, …
    assert logged_in_page.locator("h1", has_text="Modifier des données").is_visible()
    logged_in_page.locator("input[name='FT.designation']").fill("test playwright edited")

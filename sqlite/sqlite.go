...
func InsertRowInFinanceTracker(ft *FinanceTracker) (int64, error) {
	...
	result, err := db.ExecContext(
		context.Background(),`
		INSERT INTO financeTracker (sheetsId, paymentMethod, product, priceIntx100, category)
		VALUES (?,?,?,?,?);
		`, 
		ft.GofiID, ft.PaymentMethod, ft.Product, ft.PriceIntx100, ft.Category,
	)
...
func RunSQLite() {
	...
	result, err := db.Exec(`INSERT INTO financeTracker(sheetsId, paymentMethod, product, priceIntx100, category) SELECT 'sheet1', 'cb', 'test', 16.52, 'catego';`,)
	id, err := result.LastInsertId()
	if err != nil {
		fmt.Printf("err: %v\n", err)
		fmt.Printf("id : %v\n", id)
	...
	rows, err := db.QueryContext(
		context.Background(),
		// `SELECT id, day, paymentMethod, product, priceIntx100, category, commentFloat, CommentString
		// FROM financeTracker ORDER BY id DESC LIMIT 2;`)
		`
			SELECT id, sheetsId, year, month, day, paymentMethod, product, priceIntx100, category, commentFloat, CommentString, checked, dateChecked, sentToSheets
			FROM financeTracker 
			ORDER BY id DESC 
			LIMIT 3
		;`)
	...
	for rows.Next() {
		...
		// err = rows.Scan(&financeTracker.ID, &financeTracker.Day, &financeTracker.PaymentMethod, &financeTracker.Product, &financeTracker.PriceIntx100, &financeTracker.Category,
		...
		err = rows.Scan(&financeTracker.ID, &financeTracker.GofiID, &financeTracker.Year, &financeTracker.Month, &financeTracker.Day, 
			&financeTracker.PaymentMethod, &financeTracker.Product, &financeTracker.PriceIntx100, &financeTracker.Category, 
		...
		fmt.Printf("line: %v %v %v %v %v\n", financeTracker.ID, financeTracker.PaymentMethod, financeTracker.Product, financeTracker.PriceIntx100, financeTracker.Category)
